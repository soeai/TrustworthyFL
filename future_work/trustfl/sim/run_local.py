...
def run(cfg: dict):
    ...
    for rnd in range(1, cfg["rounds"] + 1):
        ...
        if defense.name in ("fltrust", "ecf", "unified"):
        ...
        if defense.name == "unified":          # unified also needs the ECF signal
            ctx.attribution_fn = make_attribution_fn(
                ...
                ig_steps=cfg.get("ig_steps", 16), gshap_samples=cfg.get("gshap_samples", 16),
                gshap_stdev=cfg.get("gshap_stdev", 0.1), gshap_seed=cfg.get("seed", 0),
