...
# 6 defenses x 10 attacks = 60 cells, 2-GPU, resumable.
...
run_one() { local gpu=$1 atk=$2 def=$3 log="$OUT/fmnist/${atk}__${def}.log"
...
worker() { local gpu=$1 s=$2 i; for ((i=s;i<${#CELLS[@]};i+=2)); do IFS='|' read -r a d <<<"${CELLS[$i]}"; run_one "$gpu" "$a" "$d"; done; }
echo "=== fmnist_grid start $(date) — ${#CELLS[@]} cells ==="|tee -a "$PROG"
